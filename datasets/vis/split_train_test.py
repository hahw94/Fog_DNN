...
from datasets.vis.util import (createFolder,
                               using_data_name_list)

from datasets.vis.preprocess import (import_data,
                                     transform_data)
...
def fog_data_train_test_split(data_path, save_path, obs_point, pre_data_list, vis_limit_num, target_columns, test_size):
            ...
            for data in data_list:
                try:
                    preprocessed_data = pd.concat([preprocessed_data,
                                                   transform_data(remove_nan_data, data, preprocessor_method)], axis=1)
    ...
    if vis_limit_num == 0:
        ...
        preprocessed_data_vis = preprocessed_data[preprocessed_data[target_columns] < vis_limit_num]

    input_data = preprocessed_data_vis[preprocessed_data_vis.columns.difference([target_columns, 'Fog_30'])]
